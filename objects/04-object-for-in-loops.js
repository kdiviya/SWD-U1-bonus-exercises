...
for (let obj in lunchBox){
	if (obj.includes("side")){
		console.log(lunchBox[obj]);
	}
}
...
function printElement(idx) {
	for(let pantries in pantry){
		let arr = pantry[pantries];
		
		if (arr.length > idx){
			console.log(`Items at Index (${idx}): ${arr[idx]}`);
		}
	}
}
...
function printPackage(pkgType){
	for(let key in pantry){

		if (key === pkgType) {
			console.log(`Items that are ${pkgType}:`) 
				pantry[key].forEach(str => console.log(`\t- ${str}`));
		}
	}
